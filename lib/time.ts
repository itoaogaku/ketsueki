 ...
 * the common road/marathon convention - the minutes part isn't required to
 * be zero-padded, e.g. "1:4:15" for 1h04m15s, a common manual-entry slip).
 * Non-time text in the same column (e.g. "途中棄権" for a DNF) simply
 * doesn't match either pattern. */
const MIN_SEC_PATTERN = /^(\d{1,4}):([0-5]\d)(?:\.(\d+))?$/;
const HOUR_MIN_SEC_PATTERN = /^(\d{1,2}):(\d{1,2}):([0-5]\d)(?:\.(\d+))?$/;

/** A trailing annotation in parentheses, e.g. "14:28.9(※参考記録)" for an
 * unofficial/time-trial result still worth scoring - stripped before
 * matching so the time itself still parses. Doesn't affect non-time text
 * like "17位" or "-", which don't match a time pattern either way. */
const TRAILING_ANNOTATION_PATTERN = /[（(][^）)]*[）)]\s*$/;

/** Parses a race time written as either "M:SS(.ms)" or "H:M:SS(.ms)" into
...
export function parseRaceTimeSeconds(text: string): number | null {
  const trimmed = text.trim().replace(TRAILING_ANNOTATION_PATTERN, "").trim();

  const hms = HOUR_MIN_SEC_PATTERN.exec(trimmed);
  if (hms) {
    const hours = Number(hms[1]);
    const minutes = Number(hms[2]);
    if (minutes > 59) return null; // e.g. "1:70:00" isn't a valid time
